    ...
    : PageModel
{
    ...
    public async Task<IActionResult> OnPost()
    {
        ...
        {
            var result = await signInManager.PasswordSignInAsync(Input.Username!, Input.Password!, Input.RememberLogin, true);
            if (result.Succeeded)
            {
    ...
    private async Task BuildModelAsync(string? returnUrl)
    {
        ...
        var providers = schemes
            ...
            .Select(x => new ViewModel.ExternalProvider(x.Name,
                x.DisplayName ?? x.Name)).ToList();

        var dynamicSchemes = (await identityProviderStore.GetAllSchemeNamesAsync())
            .Where(x => x.Enabled)
            .Select(x => new ViewModel.ExternalProvider(x.Scheme,
